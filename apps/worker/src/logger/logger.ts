...
// Resolves to apps/worker/logs/ regardless of where the process runs from
...
const LOG_FILES = {
  ...
  INFO:        path.join(LOG_DIR, "system.log"),
} as const;
...
function writeLog(
  ...
  if (category === "ERROR") {
    console.error(`${ts} ${cat} ${message}${metaStr}`);
  } else {
    console.log(`${ts} ${cat} ${message}${metaStr}`);
  }
...
export const logger = {
  /**
   * API SERVICE logs — HTTP request/response tracking.
   * Used by the API service requestLogger middleware.
   */
  ...
  /**
   * WORKER logs — BullMQ job lifecycle events.
   * Use for: job received, job stage transitions, job completed/failed.
   */
  ...
  /**
   * SYSTEM logs — infrastructure events: server start, Redis, Socket.IO.
   */
  ...
  /**
   * ERROR logs — caught exceptions.
   * Automatically extracts name, message, and stack from Error instances.
   */
  ...
  /**
   * INFO logs — general-purpose informational messages.
   */
